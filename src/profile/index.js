...
const UserProfile = () => {
    ...
    const isPostsActive = location.pathname.split("/")[3] === undefined;
    const isCommentsActive = location.pathname.split("/")[3] === "comments";
        ...
        <>
            ...
            {time.getFullYear()}-{time.getMonth()}-{time.getDate()}{" "}
                ...
                <div className="mt-3 ms-3">
                    <span className="text-secondary fw-bold">Following</span>{" "}<span
                    className="fw-bold">{user.following.length}</span>
                    <span className="text-secondary fw-bold ms-3">Followers</span>{" "}<span
                    className="fw-bold">{user.followers}</span>
                    <span className="text-secondary fw-bold ms-3">Role</span>{" "}<span
                    className="fw-bold">{user.role}</span>
                </div>
            </div>
            <br/>
            <NavigationBar />
            {isPostsActive && <UserPost uid={userId} />}
            {isCommentsActive && <CommentItem uid={userId} />}
        </>
    );
};
